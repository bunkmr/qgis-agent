...
.. note::
   本地 / 自托管模型（Ollama、vLLM、llama.cpp 等 OpenAI 兼容接口）**API Key 可留空**，
   只需填写正确的端点（如 ``http://localhost:11434/v1``）。若服务位于 Cloudflare 后返回
   ``403 PermissionDenied``，插件会自动附加浏览器 ``User-Agent`` 绕过 Bot 防护，无需手动改配置。
